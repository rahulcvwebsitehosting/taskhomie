...
export function useAgent() {
  ...
  const submit = useCallback(async (overrideText?: string, contextScreenshot?: string, overrideMode?: string) => {
    ...
    try {
      console.log("[useAgent] invoking run_agent:", { text: text.slice(0, 50), provider: currentProvider, model: selectedModel, mode, voiceMode: currentVoiceMode, conversationId });
      await invoke("run_agent", { instructions: text, model: selectedModel, provider: currentProvider, mode, voiceMode: currentVoiceMode, history, contextScreenshot: contextScreenshot ?? null, conversationId });
    } catch (error) {
