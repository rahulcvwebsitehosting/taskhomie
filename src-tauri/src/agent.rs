use crate::api::{ProviderClient, ApiError, ContentBlock, ImageSource, Message, StreamEvent, ToolResultContent};
use crate::providers;
...
impl Agent {
    ...
    pub async fn run(
        ...
        *self.computer.lock().await = Some(computer);
        ...
        let client = if providers::is_anthropic(&provider) {
            ProviderClient::Anthropic(crate::api::AnthropicClient::new(api_key, model.clone()))
        } else {
            let provider_info = providers::get_provider(&provider)
                .ok_or_else(|| AgentError::NoApiKey)?;
            ProviderClient::OpenAI(crate::api::OpenAICompatibleClient::new(
                api_key,
                model.clone(),
