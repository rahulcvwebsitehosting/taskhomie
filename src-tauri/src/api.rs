...
/// Unified client that dispatches to the correct provider
...
impl ProviderClient {
    pub async fn send_message_streaming(
        ...
        match self {
            ProviderClient::Anthropic(c) => c.send_message_streaming(messages, event_tx, mode, voice_mode).await,
            ProviderClient::OpenAI(c) => c.send_message_streaming(messages, event_tx, mode, voice_mode).await,
        }
    }
}
...
impl OpenAICompatibleClient {
    ...
    fn build_tools(&self, mode: AgentMode) -> Vec<serde_json::Value> {
        ...
        match mode {
            AgentMode::Computer => {
                tools.push(serde_json::json!({
                    ...
                    "function": {
                        "name": "computer",
                        "description": "Control the computer screen - click, type, scroll, take screenshots",
                        "parameters": {
                            "type": "object",
                            "properties": {
                                "action": {
                                    "type": "string",
                                    "enum": ["left_click", "right_click", "double_click", "type", "scroll", "key", "screenshot", "mouse_move"],
        ...
        tools.push(serde_json::json!({
            ...
            "function": {
                "name": "speak",
                "description": "Speak to the user via text-to-speech",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "text": {
                            "type": "string",
                            "description": "Natural spoken text"
                        }
                    },
                    "required": ["text"]
                }
    ...
    pub async fn send_message_streaming(
                            ...
                            if prev_msg.role == "assistant" {
                                for prev_block in &prev_msg.content {
                                    if let ContentBlock::ToolUse { id, name, .. } = prev_block {
                                        if id == tool_use_id {
                                            // add tool result
                                            let result_text = content.iter().map(|c| {
                                                match c {
        ...
        let tools = self.build_tools(mode);
        println!("[openai-compat] Sending {} tools to {}", tools.len(), self.provider_name);
        ...
        let mut current_tool_calls: Vec<(String, String, String)> = Vec::new(); // (id, name, arguments)
        let mut buffer = String::new();
        let mut usage = Usage::default();
                                    ...
                                    for tc in tool_calls {
                                        ...
                                        if let Some(func) = tc.get("function") {
                                            if let Some(name) = func.get("name").and_then(|n| n.as_str()) {
                                                if !name.is_empty() {
                                                    current_tool_calls[idx].1 = name.to_string();
