...
fn get_screen_info() -> &'static ScreenInfo {
    static FALLBACK: ScreenInfo = ScreenInfo { width: 1920.0, height: 1080.0, menubar_height: 0.0, scale: 1.0 };
    &FALLBACK
}
...
async fn run_agent(
    ...
    println!("[taskhomie] run_agent called with: {} (provider: {}, model: {}, mode: {:?}, voice: {}, history: {} msgs, screenshot: {}, conv: {:?})",
        instructions, provider, model, mode, voice, history.len(), context_screenshot.is_some(), conversation_id);
    ...
    tokio::spawn(async move {
        let agent_guard = agent.lock().await;
        match agent_guard.run(instructions, model, provider, mode, voice, history, context_screenshot, conversation_id, app_handle).await {
